...
mask_root = './dataset/test_data/CHAMELEON/GT'
pred_root = './test_maps/CHAMELEON'
...
for mask_name in tqdm(mask_name_list, total=len(mask_name_list)):
    ...
    mask_name_for_pred = mask_name.replace(".png", ".png")
    ...
    if mask is None:
        ...
        continue  # Skip this iteration if mask is not loaded correctly
    pred = cv2.imread(pred_path, cv2.IMREAD_GRAYSCALE)
    if pred is None:
        print(f"Error reading prediction: {pred_path}")
        continue  # Skip this iteration if prediction is not loaded correctly
    if len(pred.shape) != 2:
