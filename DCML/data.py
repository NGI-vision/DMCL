...
def expand_with_felzenszwalb(image, pseudo_labels):
    # 使用Felzenszwalb图像分割算法
    segments = felzenszwalb(image, scale=100, sigma=0.5, min_size=50)  # 使用 Felzenszwalb 算法
    expanded_labels = np.zeros_like(pseudo_labels)
    
    # 将伪标签应用到 Felzenszwalb 的超像素区域
    for i in range(1, np.max(segments) + 1):
        expanded_labels[segments == i] = pseudo_labels[segments == i].max()  
...
class SalObjDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 替换原有的SLIC
        SS_map = felzenszwalb(np_img, scale=100, sigma=0.5, min_size=50)
        SS_map_depth = felzenszwalb(np_depth, scale=100, sigma=0.5, min_size=50)
...
class test_dataset:
    def __init__(self, image_root, gt_root, depth_root, edge_root, testsize):
        ...
        self.edges = [edge_root + f for f in os.listdir(edge_root) if f.endswith('.png')]  # 添加边缘图像路径
        self.images = sorted(self.images)
        self.gts = sorted(self.gts)
        self.depths = sorted(self.depths)
        self.edges = sorted(self.edges)  # 确保 edges 路径也被排序
        ...
        self.edge_transform = transforms.Compose([transforms.Resize((self.testsize, self.testsize)), transforms.ToTensor()])  # 添加对边缘图像的转换
        self.size = len(self.images)
        self.index = 0
    ...
    def load_data(self):
        ...
        return image, gt, depth, name, np.array(image_for_post), edge 
